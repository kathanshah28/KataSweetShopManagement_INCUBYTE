...
def add_sweet():
    ...
    if request.method == 'POST':
        try:
            # sweet_id is now generated by the model, so we don't get it from the form
            ...
            flash(str(e), 'danger') # Handles bad number format
...
@app.route('/sort')
def sort_sweets():
    """Sorts sweets by a given key."""
    sort_key = request.args.get('by', 'name')
    sorted_sweets = shop.sort_by(sort_key)
    return render_template('index.html', sweets=sorted_sweets, sort_key=sort_key)

@app.route('/delete/<string:sweet_id>', methods=['POST'])
def delete_sweet_route(sweet_id):
    """Handles deleting a sweet."""
    if 'username' not in session:
...
@app.route('/restock/<string:sweet_id>', methods=['GET', 'POST'])
def restock_sweet_route(sweet_id):
    """Page for restocking a specific sweet."""
    if 'username' not in session:
    ...
    if request.method == 'POST':
        try:
            ...
            flash(f"Successfully restocked {sweet['name']}.", 'success')
            return redirect(url_for('index'))
        except (KeyError, ValueError) as e:
            flash(f"Restock failed: {e}", 'danger')
            
    return render_template('restock.html', sweet=sweet)
